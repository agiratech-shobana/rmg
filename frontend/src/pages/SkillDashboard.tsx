...
const [snackbar, setSnackbar] = useState<{ open: boolean; message: string; severity: SnackbarSeverity }>({
    ...
    const fetchSkillData = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/skills/employee-count`);
    ...
    const handleAddSkill = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/skills`, { name: newSkillName.trim() });
